...
public class LoginServiceImpl implements LoginService {
    ...
    @Resource
    private WechatChannelConfigService channelConfigService;


    @Override
    public void login() {

        WechatChannelConfigDTO channelConfig = channelConfigService.getConfig();
        OkhttpUtil.baseUrl = channelConfig.getBaseUrl();
        if (channelConfig.getAppId() == null || channelConfig.getAppId().isEmpty()) {
            //第一次登录
            handleNewConfig();
            return;
    ...
    private void handleExistingConfig() {
        ...
        WechatChannelConfigDTO channelConfig = channelConfigService.getConfig();
        OkhttpUtil.token = channelConfig.getToken();
        JSONObject onlineStatus = LoginApi.checkOnline(channelConfig.getAppId());
        if (isStillOnline(onlineStatus)) {
            log.info("AppId : {} 保持在线状态，跳过登录流程", channelConfig.getAppId());
            return;
        }
        log.info("AppId : {} 已离线，开始执行登录流程", channelConfig.getAppId());
        performLoginFlow(channelConfig.getAppId());
    ...
    private void handleNewConfig() {
        ...
        botConfigService.saveConfig(botConfig);
    }
    ...
    public void getToken() {
        ...
        if (response.getInteger("ret") == 200) {
            ...
            updates.setToken(OkhttpUtil.token);
    ...
    public Boolean checkStatus(Map<String, String> map) {
        ...
        while (retryCount < maxRetries) {
            ...
            if (statusCode == 2) {
                ...
                WechatChannelConfigDTO updates = new WechatChannelConfigDTO();
                updates.setAppId(map.get("appId"));
                channelConfigService.updateConfig(updates);
                return true;
    ...
    public void setCallbackUrl() {
        ...
        WechatChannelConfigDTO channelConfig = channelConfigService.getConfig();
        // 设置一下回调地址
        JSONObject callbackResponse = LoginApi.setCallback(channelConfig.getToken(), channelConfig.getCallbackUrl());
